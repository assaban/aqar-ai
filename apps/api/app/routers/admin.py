...
async def browse_channel_videos(
    ...
    from models.base import ChannelRegistration
    result = await db.execute(select(ChannelRegistration).where(ChannelRegistration.id == channel_id))
    ...
    try:
        ...
        processed_details = []
        for v in videos:
            existing = await db.execute(select(VideoSource).where(VideoSource.external_id == v.external_id))
        ...
        raise HTTPException(status_code=500, detail=f"Failed to scan channel targets: {str(e)}")